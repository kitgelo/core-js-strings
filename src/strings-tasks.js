...
function removeFirstOccurrences(str, value) {
  ...
  const word = value.length + start;
  return str.includes(value) ? `${str.slice(0, start)}${str.slice(word)}` : str;
}
...
function sumOfCodes(/* str */) {
  // let char = 0;
  // if (str) {
  //   str.split('').forEach((index) => (char += index.charCodeAt()));
  // }
  // return char;
...
function isPalindrome(/* str */) {
  ...
  //   .toLowerCase()
  //   .replaceAll(' ', '');
  // if (str.toLowerCase().replaceAll(' ', '') === reversStr) {
  //   return true;
  // }
  // return false;
  throw new Error('Not implemented');
...
function invertCase(str) {
  const word = str.split('').map((index) => {
